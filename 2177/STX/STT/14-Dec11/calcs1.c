...
int readItemFile(struct ItemRec items[], const char* filename) {
...
void ShoppingCost(void) {
  ...
  double total = 0.0;
  double totalTax = 0.0;
  if ((cnt = readItemFile(I, "items.txt")) > -1) {
    // printReport(I, cnt); write this function to replace the follwoing lines
    prnItemRecTitle();
    for (i = 0; i < cnt; i++) {
      prnItemRec(I[i]);
